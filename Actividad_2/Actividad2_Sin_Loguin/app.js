...
// ==================== RETO TÉCNICO: VARIABLES Y FUNCIONES ====================
// Array para almacenar productos del reto técnico
let retoProducts = JSON.parse(localStorage.getItem('daycor_reto_products')) || [];

// Función para renderizar productos del reto técnico
function renderRetoProducts() {
    const container = document.getElementById('retoProductsGrid');
    if (!container) return;
    
    if (retoProducts.length === 0) {
        container.innerHTML = `
            <div class="empty-products-msg">
                <i class="fas fa-box-open"></i>
                <p>No hay productos. ¡Crea tu primer producto usando el formulario!</p>
            </div>
        `;
        return;
    }
    
    container.innerHTML = retoProducts.map((product, index) => {
        const hasDiscount = product.originalPrice !== product.finalPrice;
        return `
            <div class="reto-product-card" data-index="${index}">
                ${hasDiscount ? `<div class="discount-badge">🔥 ${product.discountPercentage}% OFF</div>` : ''}
                <img src="${product.imageUrl}" alt="${product.title}" onerror="this.src='https://placehold.co/300x200/cccccc/white?text=Sin+Imagen'">
                <div class="reto-product-info">
                    <h3>${escapeHtml(product.title)}</h3>
                    <p>${escapeHtml(product.description)}</p>
                    <div class="reto-product-price">
                    ...
                    <button class="btn-delete-reto" onclick="deleteRetoProduct(${index})">
...
// Función para guardar producto del reto técnico
function saveRetoProduct() {
    const imageUrl = document.getElementById('retoImageUrl')?.value || '';
    const title = document.getElementById('retoTitle')?.value || '';
    const description = document.getElementById('retoDescription')?.value || '';
    const price = parseFloat(document.getElementById('retoPrice')?.value) || 0;
    const coupon = document.getElementById('retoCoupon')?.value?.toUpperCase() || '';
    ...
    // Lógica del cupón DESC50 = 50% de descuento
    const DISCOUNT_COUPON = 'DESC50';
    let finalPrice = price;
    let discountPercentage = 0;
    
    if (coupon === DISCOUNT_COUPON) {
        finalPrice = price * 0.5; // 50% de descuento
        discountPercentage = 50;
        showNotification('🎉 Cupón DESC50 aplicado: 50% de descuento', 'success');
    } else if (coupon !== '') {
        showNotification('⚠️ Cupón no válido. El cupón válido es DESC50', 'error');
    }
    
    const newProduct = {
        id: Date.now(),
        imageUrl: imageUrl || 'https://placehold.co/300x200/cccccc/white?text=Producto',
        title: title,
    ...
    retoProducts.push(newProduct);
    localStorage.setItem('daycor_reto_products', JSON.stringify(retoProducts));
    renderRetoProducts();
    
    // Limpiar formulario
    document.getElementById('retoTitle').value = '';
    document.getElementById('retoDescription').value = '';
    document.getElementById('retoPrice').value = '';
    document.getElementById('retoCoupon').value = '';
    
    showNotification('✅ Producto guardado exitosamente', 'success');
}

// Función para eliminar producto del reto técnico
function deleteRetoProduct(index) {
    if (confirm('¿Estás seguro de eliminar este producto?')) {
        retoProducts.splice(index, 1);
        localStorage.setItem('daycor_reto_products', JSON.stringify(retoProducts));
        renderRetoProducts();
...
function updateRetoPreview() {
    const title = document.getElementById('retoTitle')?.value || 'Título del producto';
    const description = document.getElementById('retoDescription')?.value || 'Descripción del producto...';
    const price = parseFloat(document.getElementById('retoPrice')?.value) || 0;
    const coupon = document.getElementById('retoCoupon')?.value?.toUpperCase() || '';
    const imageUrl = document.getElementById('retoImageUrl')?.value || 'https://picsum.photos/200/150';
    ...
    const previewCard = document.getElementById('retoPreviewCard');
    if (previewCard) {
        previewCard.innerHTML = `
            <img src="${imageUrl}" alt="Vista previa" onerror="this.src='https://placehold.co/300x200/cccccc/white?text=Vista+Previa'">
            <div class="preview-info">
                <h4>${escapeHtml(title)}</h4>
                <p>${escapeHtml(description.substring(0, 60))}${description.length > 60 ? '...' : ''}</p>
                <div class="preview-price">💰 $${formatPrice(finalPrice)}</div>
            </div>
        `;
    }
}
...
if (coupons.length === 0) {
    coupons.push({ id: 1, code: "DESC50", discount: 50, startDate: "2024-01-01", endDate: "2025-12-31", active: true });
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    // Eventos del Reto Técnico
    const retoSaveBtn = document.getElementById('retoSaveBtn');
    if (retoSaveBtn) retoSaveBtn.addEventListener('click', saveRetoProduct);
    
    const retoImageUrl = document.getElementById('retoImageUrl');
    const retoTitle = document.getElementById('retoTitle');
    const retoDescription = document.getElementById('retoDescription');
    const retoPrice = document.getElementById('retoPrice');
    const retoCoupon = document.getElementById('retoCoupon');
    
    if (retoImageUrl) retoImageUrl.addEventListener('input', updateRetoPreview);
    if (retoTitle) retoTitle.addEventListener('input', updateRetoPreview);
    if (retoDescription) retoDescription.addEventListener('input', updateRetoPreview);
    if (retoPrice) retoPrice.addEventListener('input', updateRetoPreview);
    if (retoCoupon) retoCoupon.addEventListener('input', updateRetoPreview);
...
window.deleteRetoProduct = deleteRetoProduct;
window.saveRetoProduct = saveRetoProduct;